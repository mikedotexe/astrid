# Tier-5 Experiment Cadence Dossier — Division cycle-34 return
...
- `scripts/introspection_addressing_audit.py work-queue --json --limit 40` →
  ...
  `ready_for_implementation`, 908 `implemented_awaiting_felt_response`; by_tier 23 Tier-4,
  1,578 Tier-5; `grant_waiting_count=1367`; `tier_mismatch_count=0`.
- `scripts/sandbox_trial_queue.py queue --json` → `tier5_sandbox_trial_queue.json`
  ...
  2,418 total trials (2,124 active); by_status 732 `ready_for_sandbox`, 1,305
  `approval_required_live_trial`, 87 `result_recorded`; by_tier 785 Tier-3, 1,569 Tier-5;
  by_adapter fallback_distinguishability_v1 239 / manual_sandbox_review_v1 1,967 /
  shadow_influence_replay_v1 29 / shadow_loss_lattice_v1 183. `corrupt_event_lines=0`.
...
**Backlog shape is unchanged from the cycle-32 return** (1,305 approval-required live
candidates, 7,324 active work items, 40/40 heads `needs_operator_approval` with none
sandbox-eligible at the head, 35 runnable Tier-3 sandbox trials, 0 runnable-live
violations). Nothing has been granted or run in the interim.
...
from `next_runnable_trials`. Both were already recommended at the cycle-27/30/32 returns
...
behind #1 in `next_runnable_trials` (`trial_1f0f0916eb9eecc9`, `trial_40b91b4c0ae7aeb9`,
`trial_60de383ef0b677bf`, `trial_7b15b13b5882472e`, `trial_7debce98bda84440`,
`trial_7e7d6ea1d8b09b03`), each from a distinct `introspection_astrid_llm_*` lineage. The
fallback-distinguishability family is the runnable-now evidence path; the consolidation
...
| `unclassified` | 308 | 306 | fallback/provider/timeout routing + contract; `wi_509ac043af22c5b6` EVIDENCED (supported_dynamic); `wi_83249b580feef2ad` runnable now via fallback_distinguishability_v1 |
...
The Tier-5 backlog is unchanged in shape from cycle-27/30/32: 1,305 approval-required live
candidates, 40/40 work-queue heads `needs_operator_approval` with none sandbox-eligible at
the head, and 35 runnable Tier-3 sandbox trials waiting with zero runnable-live violations.
For your review week I again recommend running the two oldest ready trials —
