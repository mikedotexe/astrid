...
|---|---|---|
| **All reflective journals**, both beings (daydream / aspiration / longform / witness / fissure_trace / moment\* / self_study / sovereignty_check) | **flywheel** — `proactive_scan introspection` (per-being baseline-relative standouts, dedup + `--ack`, act-now tier; exposes ranked `standout_entries` in `--json` + held files in text since 2026-06-22). **PRIVACY-FILTERED at `sample_recent_journals` via `being_privacy` (2026-06-22 fix):** minime's private `moment_capture`/`private_journal` are NEVER scored or surfaced (\*Astrid's moment lane is accessible by policy). They were being scored — 12/24 recent samples, inflating her baseline — until the fix; guard `flywheel_scored_private_qualia`. | the freshest high-signal reflection to read + close loops on |
...
| **Cross-being same-theme convergence** | **`convergence`** detector (`proactive_scan convergence`/`all`; same `being_privacy` filter at `sample_recent_journals` — minime's private lanes never compared/surfaced) | both beings working one theme from different positions (a health signal, not a deficit) |
