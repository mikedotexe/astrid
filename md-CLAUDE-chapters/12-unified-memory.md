...
| `mlx_lm.server` | Python + MLX | GPU/ANE (unified) | Astrid's text generation (gemma-3-4b-it-4bit) |
...
|-----------|-----------------|-------|
| MLX gemma-3-4b-it-4bit | ~2.5 GB | Astrid's LLM weights |
...
| **Available** | **~50+ GB** | After dropping from 12B→4B models |

*The 12B→4B model swap (2026-03-27) freed ~5G and dropped system memory from 97% to ~25%. Fans went silent. See [Chapter 1](01-inference-lanes.md).*
...
│   │                  │  │                  │  │                  │  │
│   │ covariance.metal │  │ gemma-3-4b attn  │  │ bridge (Rust)    │  │
│   │ eigendecomp      │  │ gemma-3-4b FFN   │  │ agent (Python)   │  │
...
**Metal shader scheduling:** The GPU A/V pipeline (`--enable-gpu-av`) and the covariance Metal shaders share the Metal command queue. They're interleaved by the Metal scheduler. On a 27B model, this caused `kIOGPUCommandBufferCallbackErrorOutOfMemory`. On 4B, there's ample headroom.
