...
fn continue_never_yields_a_recovery_input_in_any_reader_state() {
    // Valid Rust comments avoid repeatedly exercising parser error recovery.
    // This remains long enough that the first page cannot reach end of file.
    ...
    let pending_output = reader.prepare(open()).unwrap();
    let pending = pending_output.page.as_ref().unwrap();
    ...
    reader
        .delivered(&pending.id, &wire(&pending_output.text), &response())
        .unwrap();
