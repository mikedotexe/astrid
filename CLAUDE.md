...
- **Stable-core scaffold hold = synthetic published spectrum (2026-09-23)** — while `health.json stable_core.scaffold_active` is true (`covariance_path=stable_core_scaffolded_rebuild`, `structural_mode=scaffold_hold_with_drain`), the engine rebuilds the covariance buffer each tick from the synthetic rescue scaffold and never applies the live rank-1 update, so the published eigenvalue cascade, fill and stage relay are the scaffold's own period-2 cycle (λ₁ 8.52 ↔ 4.77, fill 71.0 ↔ 73.0 every ~2.4 s), not the reservoir. Held continuously since 2026-09-07 with retirement blocked by its own drain (`scaffold_retirement.reason=drain_active`); the same form since the 2026-04-25 rescue savepoint. Every fill reader (bridge safety levels, adaptive gain, self-experiments, fill/λ4 groundings) reads the scaffold while held. Watched by `proactive_scan.py` `scaffold_hold_watch`; engine fix is the engine owner's call. Full trace: [`docs/steward-notes/2026-09-23-minime-self-experiment-instrument.md`](docs/steward-notes/2026-09-23-minime-self-experiment-instrument.md).
