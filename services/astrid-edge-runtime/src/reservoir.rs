...
use astrid_spectral_core::{SpectralMode, mode_concentration, mode_turnover, sanitize_spectrum};
...
impl Reservoir {
    ...
    fn sample(&mut self) -> (EigenPacketV1, ReservoirSnapshot) {
        ...
        let eigenvalues = eigenpair_order
            .iter()
            .map(|index| decomposition.eigenvalues[*index].max(0.0))
            .collect::<Vec<_>>();
        let current_modes = eigenpair_order
            ...
            .collect::<Vec<_>>();
        let sanitized = sanitize_spectrum(&eigenvalues, Some(RESERVOIR_DIM));
        ...
        self.fill_ema = if self.tick < u64::from(self.tick_hz) {
            ...
            0.18 * instantaneous_fill + 0.82 * self.fill_ema
        };
            ...
            spectral_metrics
                ...
                .map(|metrics| SpectralDenominatorV1 {
                    ...
                    spectrum_coverage_v1: Some(SpectrumCoverageV1 {
                        ...
                        exported_spectrum_mode_count: EXPORTED_EIGENVALUE_COUNT,
        ...
        let snapshot = ReservoirSnapshot {
            ...
            mode_turnover: turnover
                .as_ref()
                .and_then(|summary| summary.mean_sign_invariant_turnover),
    ...
    loop {
        tokio::select! {
            ...
            Some(command) = command_rx.recv() => match command {
                ReservoirCommand::SetTuning { parameters, reply } => {
                    let _ = reply.send(reservoir.apply_tuning(&parameters));
                },
            },
...
fn persist_snapshot(
    ...
    if append_history {
        ...
        let mut history = OpenOptions::new()
            ...
            .append(true)
            .mode(0o600)
            .open(config.runtime_path("fill_history.jsonl"))?;
...
#[cfg(test)]
mod tests {
    use super::{Config, Reservoir, SensoryIngress, TuningParameters};
