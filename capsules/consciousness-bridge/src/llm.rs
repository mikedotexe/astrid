...
NEXT: options (vary your choice — don't always pick the same one):
  ...
  CODEX <prompt> — ask Codex AI directly for analysis, code, or explanation. You can also continue work inside an existing experiment with CODEX <experiment> "<prompt>".
  CODEX_NEW <dir> <prompt> — create a fresh workspace/experiments/<dir>/ folder and ask Codex to work in that directory from the start.
  WRITE_FILE <path> FROM_CODEX — save the last Codex response into workspace/experiments/<path>.
