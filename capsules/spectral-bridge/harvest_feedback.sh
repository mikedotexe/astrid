...
ASTRID_WORKSPACE="/Users/v/other/astrid/capsules/spectral-bridge/workspace"
...
echo "## STRUCTURED: Self-study review packet"
if [ -x "$SCRIPT_DIR/scripts/self_study_review.py" ] || [ -f "$SCRIPT_DIR/scripts/self_study_review.py" ]; then
    python3 "$SCRIPT_DIR/scripts/self_study_review.py" --limit 5 --print-summary 2>/dev/null | sed 's/^/  /'
else
    echo "  self_study_review.py not found"
fi
echo ""
...
MISSING=""
for p in "minime run" "spectral-bridge-server" "coupled_astrid_server" "reservoir_service" "autonomous_agent" "astrid_feeder" "minime_feeder" "camera_client" "mic_to_sensory" "perception.py"; do
