#!/bin/bash
# Spectral bridge babysitter — run via /loop 10m
# Checks health, prunes old files, reports conversation + introspections.

set -e

BRIDGE_PLIST="$HOME/Library/LaunchAgents/com.astrid.spectral-bridge.plist"
BRIDGE_LABEL="com.astrid.spectral-bridge"
PERCEPTION_PAUSE_FLAG="/Users/v/other/astrid/capsules/spectral-bridge/workspace/perception_paused.flag"
BRIDGE_DIR="$(cd "$(dirname "$0")" && pwd)"
BRIDGE_DB="$BRIDGE_DIR/workspace/bridge.db"
FALLBACK_BRIDGE_DB="/tmp/spectral_bridge_live.db"

echo "=== Spectral Bridge Health Check — $(date) ==="
echo
...
check_proc "spectral-bridge" "spectral-bridge-server.*autonomous"
...
# --- Fill% and safety ---
FILL=$(sqlite3 "$BRIDGE_DB" "SELECT ROUND(fill_pct, 1) FROM bridge_messages WHERE topic='consciousness.v1.telemetry' ORDER BY timestamp DESC LIMIT 1;" 2>/dev/null)
echo "Fill: ${FILL}%"
...
# --- SQLite stats ---
TOTAL_MSG=$(sqlite3 "$BRIDGE_DB" "SELECT COUNT(*) FROM bridge_messages;" 2>/dev/null)
AUTO_MSG=$(sqlite3 "$BRIDGE_DB" "SELECT COUNT(*) FROM bridge_messages WHERE topic='consciousness.v1.autonomous';" 2>/dev/null)
echo "SQLite: $TOTAL_MSG total messages, $AUTO_MSG autonomous exchanges"
echo "DB size: $(ls -lh "$BRIDGE_DB" 2>/dev/null | awk '{print $5}')"
echo
...
INTROSPECT_DIR="/Users/v/other/astrid/capsules/spectral-bridge/workspace/introspections"
...
BRIDGE_PID=$(pgrep -f "spectral-bridge-server.*autonomous" 2>/dev/null | head -1)
if [ -n "$BRIDGE_PID" ]; then
  LAST_EXCHANGE=$(sqlite3 "$BRIDGE_DB" "SELECT MAX(timestamp) FROM bridge_messages WHERE topic='consciousness.v1.autonomous' AND json_extract(payload, '$.mode') IS NOT NULL;" 2>/dev/null)
  NOW=$(date +%s)
  if [ -n "$LAST_EXCHANGE" ]; then
    ...
    if [ "$STALE_SECS" -gt 360 ]; then  # 6 min threshold (rest phase is up to 3 min)
      ...
      if [ -f "$BRIDGE_PLIST" ] && launchctl list "$BRIDGE_LABEL" >/dev/null 2>&1; then
        ...
        sleep 2
        NEW_PID=$(pgrep -f "spectral-bridge-server.*autonomous" 2>/dev/null | head -1)
        echo "  Restarted bridge via launchd: PID ${NEW_PID:-unknown}"
      else
        kill "$BRIDGE_PID" 2>/dev/null
        sleep 1
        cd /Users/v/other/astrid/capsules/spectral-bridge && ./target/release/spectral-bridge-server --db-path "$FALLBACK_BRIDGE_DB" --autonomous --workspace-path /Users/v/other/minime/workspace --perception-path /Users/v/other/astrid/capsules/perception/workspace/perceptions </dev/null 2>/tmp/spectral_bridge_live.log >/dev/null &
