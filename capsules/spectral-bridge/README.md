# Astrid-Minime Bridge

A capsule that bridges Astrid's IPC event bus with minime's spectral
reservoir runtime, creating a bidirectional communication channel with
...
Astrid Kernel                          minime Engine
     IPC EventBus                        ESN (128D)
                        +-----------+
  legacy topic        |  Bridge   |    ws://7878 (telemetry out)
  telemetry     <------+  Capsule  +----> eigenvalues, fill_ratio
  control       ------>|           +----> ws://7879 (sensory in)
  semantic      ------>|  SQLite   |
  status        <------+  log      |
                        +-----------+
```
...
# Run (connects to minime on default ports; binary name is legacy)
