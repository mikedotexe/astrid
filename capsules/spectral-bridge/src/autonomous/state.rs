...
impl ConversationState {
                        ...
                        && *c2 >= pair_min
                    {
                        ...
                        let hint = format!(
                            "You've been oscillating between {} and {} \
                             ...
                             FLUCTUATION_AUDIT to ask whether the fluctuation remains inhabitable, \
                             BRACE_AUDIT to distinguish rest from post-spike bracing, RESISTANCE_GRADIENT to map groan/resistance as a vector, or \
                             SHADOW_FIELD / DECAY_MAP / FOLD_HOLD / RESONANCE_FORECAST to change \
    ...
    /// Legacy checkpoint compatibility; durable windows now own admission.
    #[cfg(test)]
        ...
        if inbox_present && mode_name == "dialogue_fallback" {
            ...
            if self.inbox_forced_fallback_streak == INBOX_FORCED_FALLBACK_LIMIT {
                warn!(
                    ...
                    "{} chosen mailbox turns ended without a completion; letters remain pending for another chosen window",
