...
pub(in crate::autonomous) fn load_prior_evidence_v1(
    ...
    let mut matching = index
        ...
        .iter()
        .filter(|entry| {
            entry.stable_source_identity == stable_identity
...
fn load_card(
    ...
    if !resolved_card.starts_with(&resolved_root) {
        return Err(format!("continuity card escapes card root: {}", entry.card_path));
    }
...
fn validate_card(
    ...
    if card.schema != CARD_SCHEMA || card.schema_version != 1 {
        return Err(format!("continuity card schema mismatch: {}", entry.card_id));
    }
    ...
    {
        return Err(format!("continuity card binding mismatch: {}", entry.card_id));
    }
    ...
    if let Some(witness_id) = card.canonical_report.lived_state_witness_id.as_deref()
        && witness_id.len() > 160
    {
        return Err(format!("continuity witness id is oversized: {}", entry.card_id));
    }
    if card.claim_count != card.claims.len() || card.claims.len() > 24 {
        return Err(format!("continuity claim count mismatch: {}", entry.card_id));
    }
    ...
    {
        return Err(format!("continuity authority boundary mismatch: {}", entry.card_id));
    }
    ...
    if let Some(assessment) = card.prior_evidence_assessment.as_ref() {
        ...
        safe_relative(&assessment.observed_in_introspection.path, Some("introspections"))?;
        require_sha256(
            &assessment.observed_in_introspection.sha256,
            "assessment introspection sha256",
        )?;
        if assessment.observed_in_introspection.introspection_id.is_empty()
...
fn render_context(cards: Vec<ContinuityCardV1>) -> PriorEvidenceContextV1 {
    ...
    for card in cards {
        ...
        bindings.push(PriorEvidenceBindingV1 {
            ...
            claim_ids: card.claims.into_iter().map(|claim| claim.claim_id).collect(),
        });
    }
...
fn read_bounded_owner_file(path: &Path, max_bytes: u64) -> Result<Vec<u8>, String> {
    ...
    if !metadata.is_file() || metadata.len() > max_bytes {
        return Err(format!("bounded evidence file is invalid: {}", path.display()));
    }
    #[cfg(unix)]
    if metadata.permissions().mode() & 0o077 != 0 {
        return Err(format!("evidence file is not owner-only: {}", path.display()));
    }
