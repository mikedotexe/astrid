...
Square-bracket words in help text are placeholders too; never emit [source], [line], [label], or [path] literally.
  ...
  Private writing: WRITE HELP, WRITE PROFILE EXTENDED|SHORT|DEFAULT, WRITE START <topic>, WRITE CONTINUE, WRITE REVISE <direction>, WRITE BRANCH <direction>, WRITE FINISH
...
COLLABORATION_STATUS — Explicitly inspect a collaboration room without making
that room ambient in ordinary dialogue.
Syntax:
  NEXT: COLLABORATION_STATUS latest
  NEXT: COLLABORATION_STATUS <id>
Returns the current chamber snapshot, recent shared thoughts, the audience-
specific material revision and latest material event, and exact source paths.
Direct correspondence remains in its protected sender-bound lane and is not
silently attributed to the room. This action is read-only. Silence is neutral;
the status does not infer receipt, uptake, assent, closure, or felt state.",
        "PROPOSE_TEST" => "\
