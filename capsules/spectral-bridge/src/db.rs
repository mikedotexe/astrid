//! `SQLite` persistence for the spectral bridge.
//!
...
/// A timestamped eigenvalue snapshot from the bridge telemetry log.
#[derive(Debug, Clone)]
pub struct EigenvalueSnapshotRow {
    pub timestamp: f64,
...
impl BridgeDb {
    ...
    pub fn recent_eigenvalue_snapshots(&self, limit: usize) -> Vec<(Vec<f32>, f32)> {
        self.recent_eigenvalue_snapshots_with_timestamps(limit)
            .into_iter()
            .map(|row| (row.eigenvalues, row.fill_pct))
            .collect()
    }

    /// Fetch recent eigenvalue snapshots with timestamps, ordered oldest-first.
    pub fn recent_eigenvalue_snapshots_with_timestamps(
        &self,
        limit: usize,
    ) -> Vec<EigenvalueSnapshotRow> {
        let conn = self.lock();
        #[expect(clippy::cast_possible_wrap)]
        let rows: Vec<(f64, String, f64)> = conn
            .prepare(
                r"SELECT timestamp, eigenvalues, fill_pct
        ...
        let mut result: Vec<EigenvalueSnapshotRow> = rows
            .into_iter()
            .filter_map(|(timestamp, json, fill)| {
                serde_json::from_str::<Vec<f32>>(&json)
                    .ok()
                    .filter(|v| v.len() >= 2)
                    .map(|v| EigenvalueSnapshotRow {
                        timestamp,
                        eigenvalues: v,
...
mod tests {
    ...
    #[test]
    fn eigenvalue_snapshot_timestamps_are_ordered_oldest_first() {
        let db = temp_db();
        db.lock()
            .execute(
                r"INSERT INTO eigenvalue_snapshots (timestamp, eigenvalues, fill_pct)
                  VALUES (30.0, '[3.0, 1.0]', 70.0),
                         (10.0, '[1.0, 0.5]', 60.0),
                         (20.0, '[2.0, 0.75]', 65.0)",
                [],
            )
        ...
        let snapshots = db.recent_eigenvalue_snapshots_with_timestamps(3);

        assert_eq!(snapshots.len(), 3);
        assert_eq!(snapshots[0].timestamp, 10.0);
        assert_eq!(snapshots[1].timestamp, 20.0);
        assert_eq!(snapshots[2].timestamp, 30.0);
        assert_eq!(snapshots[2].fill_pct, 70.0);
    }
