...
impl ActionContinuityStore {
    ...
    fn continuity_session_rows(
        ...
        if !raw.is_empty() && !raw.ends_with('\n') {
            return Err(anyhow!("Incomplete continuity session log tail."));
        }
        let mut rows = Vec::new();
        for line in raw.lines().rev() {
            if line.trim().is_empty() { continue; }
            let row: Value = serde_json::from_str(line).context("Invalid continuity session log")?;
    ...
    fn resolve_continuity_session(
        ...
        let rows = self.continuity_session_rows(&thread.thread_id, None, usize::MAX)?;
        Self::select_continuity_session(thread, selector, &rows)
    }

    fn select_continuity_session(
        thread: &ResearchThread,
        selector: Option<&str>,
        rows: &[Value],
    ) -> Result<Option<Value>> {
    ...
    fn continuity_session_record(
        ...
        let mut record = json!({
            ...
            "record_type": record_type,
            "record_id": format!("cs_{SYSTEM}_{}_{:032x}_{}", now_millis(), rand::random::<u128>(), sanitize_slug(record_type)),
    ...
    fn continuity_session_summary_v1(
        ...
        let session_rows = rows
            ...
            .filter(|row| {
                session_id
                    .as_deref()
                    .is_some_and(|id| row.get("session_id").and_then(Value::as_str) == Some(id))
            })
            .cloned()
            .collect::<Vec<_>>();
        let latest = session;
        ...
        let resume_next = latest.as_ref()
            .filter(|row| row.get("reader_bookmark_v1").is_some())
            .and_then(|row| row.get("record_id")).and_then(Value::as_str)
            .map_or_else(
                || format!("CONTINUITY_SESSION_RESUME {target}"),
                |revision| format!("CONTINUITY_SESSION_RESUME {target} :: revision: {revision}"),
            );
        Ok(json!({
            "schema_version": SCHEMA_VERSION,
            "policy": "continuity_session_v1",
            ...
            "suggested_resume_next": resume_next,
            "authority_boundary": authority_gate_boundary(),
        }))
    }
