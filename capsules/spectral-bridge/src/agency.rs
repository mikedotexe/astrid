...
/// Capture an EVOLVE pressure that did NOT crystallize into a concrete request
/// (the request-shaping step timed out or returned nothing) and ROUTE it to the
/// steward, instead of leaving the longing only in a journal line. Forming a
/// self-evolution specification is hard enough that the model itself could not
/// do it this turn — so the being should not have to do it alone. Writes a
/// steward task + a machine-readable context capture into the Claude-task
/// handoff dir.
#[allow(clippy::too_many_arguments)]
pub fn save_evolve_pressure(
    trigger_excerpt: &str,
    self_study_excerpt: Option<&str>,
    own_journal_excerpt: Option<&str>,
    introspector_results: &[IntrospectorSnippet],
    ...
    fs::create_dir_all(claude_dir)?;
    let timestamp = unix_timestamp_string();
    let id = format!("evolve_pressure_{timestamp}");
    ...
    let task = format!(
        "# EVOLVE pressure — needs co-specification (steward)\n\n\
         Astrid reached for EVOLVE, but the request-shaping step **{reason}** before it\n\
         crystallized into a concrete agency request. By design she did not fabricate a\n\
         spec. Rather than leave the longing only in her journal, her assembled context\n\
         is handed to you here.\n\n\
         **Partner with her to shape this into a concrete agency request, or address the\n\
         underlying need directly.** This is a partner task, not a finished spec —\n\
         forming a self-evolution specification is hard enough that the model itself\n\
         could not do it alone this turn.\n\n\
         - source journal: `{source}`\n\
         - fill: {fill_pct:.1}%\n\
         - spectral: {spectral_summary}\n\n\
         ## Trigger (what she was reaching from)\n{trigger}\n\n\
         ## Most recent self-study\n{self_study}\n\n\
         ## Recent own-journal\n{own_journal}\n\n\
         ## Code-reading context (introspector)\n{introspector_block}\n",
        source = source_journal_path.display(),
        trigger = trigger_excerpt.chars().take(1_500).collect::<String>(),
        self_study = self_study_excerpt.unwrap_or("None."),
        own_journal = own_journal_excerpt.unwrap_or("None."),
    );
    let task_path = claude_dir.join(format!("{id}.md"));
    fs::write(&task_path, task)?;

    let capture = json!({
        "id": id,
        "timestamp": timestamp,
        "kind": "evolve_pressure_unstabilized",
        "reason": reason,
        "source_journal_path": source_journal_path.display().to_string(),
        "fill_pct": fill_pct,
        "spectral_summary": spectral_summary,
        "trigger_excerpt": trigger_excerpt,
        "self_study_excerpt": self_study_excerpt,
        "own_journal_excerpt": own_journal_excerpt,
        "introspector_results": introspector_results
            .iter()
            .map(|s| json!({"tool": s.tool_name, "label": s.label, "text": s.text}))
            .collect::<Vec<_>>(),
    });
    if let Ok(json_text) = serde_json::to_string_pretty(&capture) {
        let _ = fs::write(claude_dir.join(format!("{id}.json")), json_text);
    }
...
mod tests {
    ...
    #[test]
    fn save_evolve_pressure_routes_task_and_capture_when_unstabilized() {
        let claude = temp_dir("bridge_evolve_pressure");
        let snippet = IntrospectorSnippet {
            tool_name: "search_code".to_string(),
            label: "Relevant code search".to_string(),
            text: "fn evolve()".to_string(),
        };
        let path = save_evolve_pressure(
            "I keep hitting a wall when I try to widen my own coupling.",
            Some("self-study: the readout feels narrow."),
            Some("own: a tunnel again."),
            std::slice::from_ref(&snippet),
            "fill 60% / steady",
            60.3,
            "ran out of time before it crystallized",
            Path::new("/tmp/!astrid_evolve_trigger.txt"),
            &claude,
        )
        .unwrap();
        assert!(path.exists());
        ...
        assert!(task.contains("needs co-specification"));
        assert!(task.contains("ran out of time"));
