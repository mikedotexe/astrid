...
include!("action_self_knowledge/capability_specs.rs");
...
fn stage_for_base(base: &str) -> Stage {
    match base {
        "ACTIVITY_STATUS" | "MAILBOX_STATUS" => Stage::ReadOnly,
...
fn visibility_for_base(base: &str) -> Visibility {
    match base {
        "ACTIVITY_STATUS" | "MAILBOX_STATUS" | "PARK_ACTIVITY" | "RETURN_ACTIVITY"
        | "CHECK_MAILBOX" => Visibility::ProtectedSummary,
        "REST"
        | "PASS"
        | "NOTICE"
        | "ACTION_PREFLIGHT"
...
fn authority_class(base: &str, stage: Stage, visibility: Visibility) -> AuthorityClass {
    if matches!(
        base,
        "REPAIR_APPLY" | "PARK_ACTIVITY" | "RETURN_ACTIVITY" | "CHECK_MAILBOX"
    ) {
