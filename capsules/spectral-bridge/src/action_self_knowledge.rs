...
mod tests {
    ...
    fn capability_map_includes_core_self_knowledge_actions() {
        ...
        assert!(bases.contains("FOLD_HOLD"));
        ...
        let fold_status = handle_action(&root, "CAPABILITY_STATUS", "CAPABILITY_STATUS FOLD_HOLD")
            .expect("handle")
            .expect("message");
        assert!(fold_status.contains("protected non-control fold"));
        assert!(fold_status.contains("protected_read_only"));
        let brace_status =
            handle_action(&root, "CAPABILITY_STATUS", "CAPABILITY_STATUS BRACE_AUDIT")
                .expect("handle")
                .expect("message");
