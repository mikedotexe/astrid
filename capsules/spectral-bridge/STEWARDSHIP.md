# Spectral Runtime Stewardship Agent
...
ps aux | grep -E "minime|spectral-bridge|perception|autonomous_agent|camera_client|mic_to_sensory" | grep -v grep | wc -l
...
```bash
bash /Users/v/other/astrid/capsules/spectral-bridge/harvest_feedback.sh 2>/dev/null
```
...
find /Users/v/other/astrid/capsules/spectral-bridge/workspace -type f -mmin -5 -name "*.txt" -o -name "*.json" | head -15
```
...
```bash
sqlite3 /Users/v/other/astrid/capsules/spectral-bridge/workspace/bridge.db "SELECT COUNT(*) FROM astrid_starred_memories;"
sqlite3 /Users/v/other/astrid/capsules/spectral-bridge/workspace/bridge.db "SELECT observation FROM astrid_self_observations ORDER BY timestamp DESC LIMIT 1;"
sqlite3 /Users/v/other/astrid/capsules/spectral-bridge/workspace/bridge.db "SELECT COUNT(*) FROM astrid_latent_vectors;"
```
...
| 5 | coupled_astrid_server.py | `/Users/v/other/neural-triple-reservoir/.venv/bin/python /Users/v/other/neural-triple-reservoir/coupled_astrid_server.py --port 8090 --coupling-strength 0.1 --model-memory-map --model mlx-community/gemma-4-12B-it-5bit &` | `/Users/v/other/neural-triple-reservoir` |
| 6 | spectral-bridge | `./target/release/spectral-bridge-server --db-path workspace/bridge.db --autonomous --workspace-path /Users/v/other/minime/workspace --perception-path /Users/v/other/astrid/capsules/perception/workspace/perceptions &` | `/Users/v/other/astrid/capsules/spectral-bridge` |
...
cd /Users/v/other/astrid/capsules/spectral-bridge && ./target/release/spectral-bridge-server \
...
```bash
# Astrid side
pkill -f spectral-bridge-server
...
```bash
cd /Users/v/other/astrid/capsules/spectral-bridge
cargo build --release
pkill -f spectral-bridge-server && sleep 2
./target/release/spectral-bridge-server \
  --db-path /Users/v/other/astrid/capsules/spectral-bridge/workspace/bridge.db \
  --autonomous \
  --workspace-path /Users/v/other/minime/workspace \
  --perception-path /Users/v/other/astrid/capsules/perception/workspace/perceptions &
...
- Astrid inbox: `/Users/v/other/astrid/capsules/spectral-bridge/workspace/inbox/`
...
- Astrid outbox: `/Users/v/other/astrid/capsules/spectral-bridge/workspace/outbox/`

### Verifying Health After Restart
```bash
# Count processes (expect 8)
ps aux | grep -E "minime|spectral-bridge|perception|autonomous_agent|camera_client|mic_to_sensory|mlx_lm" | grep -v grep | wc -l
...
find /Users/v/other/astrid/capsules/spectral-bridge/workspace -type f -mmin -5 | wc -l
```
...
### Architecture Notes (updated 2026-06-05)

**Inference lanes (current configured stack):**
- **Astrid live dialogue → coupled MLX** (`coupled_astrid_server.py`, `mlx-community/gemma-4-12B-it-5bit` on port 8090, bridge profile `gemma4_12b`). Token generation is bidirectionally coupled to the triple reservoir.
- **Astrid fallback/embeddings/vision → Ollama** (`gemma3:4b`, `nomic-embed-text`, `llava-llama3`).
- **Minime autonomous → Ollama** (`gemma3:12b`, fast fallback `gemma3:4b`). Optional MLX exists, but do not assume port 8090 is free in the full stack.

Run `/Users/v/other/astrid/scripts/model_stack_audit.py` before changing any
model default or interpreting older model notes. Add `--include-historical` when
you want changelog and steward-note matches included in the stale-reference scan.
...
- `/Users/v/other/astrid/capsules/spectral-bridge/src/autonomous.rs` — dialogue loop, burst/rest, mode selection, warmth blending
- `/Users/v/other/astrid/capsules/spectral-bridge/src/codec.rs` — 48D semantic encoding, SEMANTIC_GAIN, warmth vector
- `/Users/v/other/astrid/capsules/spectral-bridge/src/llm.rs` — Ollama integration, prompts
- `/Users/v/other/astrid/capsules/spectral-bridge/src/ws.rs` — WebSocket connections, safety levels

**Data sources:**
- Minime journals: `/Users/v/other/minime/workspace/journal/`
- Astrid journals: `/Users/v/other/astrid/capsules/spectral-bridge/workspace/journal/`
- Older journal history for both lives under sibling `archive/until_*` buckets inside those directories.
- Bridge DB: `/Users/v/other/astrid/capsules/spectral-bridge/workspace/bridge.db`
- Harvester: `/Users/v/other/astrid/capsules/spectral-bridge/harvest_feedback.sh`
