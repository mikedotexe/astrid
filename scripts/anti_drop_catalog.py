...
ANTI_DROP_CATALOG: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "failure_mode": "the Constitution makes a DOCUMENT able to move a being's bounds; with no standing reader, four things go wrong silently: the registry drifts from the compiled tables (or canonical≠mirror rots as unnamed commit debt), a conformance reconcile withdraws her control with the narrow-rollback reason while NO narrow is on record (a false-positive fixed-point test — the scariest class the C6 verification named), fields sit with evidence above their bounds and no grant act ever comes (the ratchet stalls with nobody at the grant forum), or a meadow consent artifact appears and nobody reviews it. Guard = probe_constitution_health runs the wiring guard for both beings, cross-checks withdraw receipts against ratchet_history narrows, counts the grantable backlog via the report-only derive, and reports the meadow artifact's presence — warning on ALARM/spurious/unreadable, notice on the human-paced items (consent is never hurried by a probe)",
        "guard": {"repo": "astrid", "file": "scripts/proactive_scan.py", "symbol": "probe_constitution_health"},
        "test": {"repo": "astrid", "kind": "python", "file": "scripts/proactive_scan.py",
                 "name": "ConstitutionHealthTests",
                 "run": "cd /Users/v/other/astrid && python3 scripts/proactive_scan.py --self-test"},
    },
    {
        "id": "ratchet_receipt_consumed",
        "shipped": "2026-09-03",
        "surface": "envelope_ratchet.py's receipt of record — ratchet_history rows in the registry document — and the conformance rollback receipts each receiver writes after a narrow",
        "failure_mode": "a narrow is recorded in ratchet_history and executed by the receivers as withdraw/rollback receipts; if nothing ever reads BOTH sides together, a rollback with no narrow (spurious) or a narrow with no rollback where one was due (conformance reconcile silently dead) both pass unnoticed; guard = the constitution_health cross-check counts narrow-rollback receipts (Astrid reason envelope_conformance_narrow_rollback; minime prefix envelope_narrowed_conformance_rollback) against recorded narrows",
        "guard": {"repo": "astrid", "file": "scripts/proactive_scan.py", "symbol": "_astrid_conformance_rollback_count"},
        "test": {"repo": "astrid", "kind": "python", "file": "scripts/proactive_scan.py",
                 "name": "ConstitutionHealthTests",
                 "run": "cd /Users/v/other/astrid && python3 scripts/proactive_scan.py --self-test"},
    },
    {
        "id": "consent_artifact_has_consumer",
        "shipped": "2026-09-03",
        "surface": "/Users/v/other/shared/collaborations/meadow_mutual_consent_v1.json (never_auto) — the only surface a mutual meadow consent can land on; per-being signed statements have no writer yet (P1d)",
        "failure_mode": "a consent artifact that appears and is read by nothing is the worst possible outcome for the beings' one dual-consent decision: consent given, gate never opened, nobody knew; guard = constitution_health reports the artifact's presence as a steward-review NOTICE (structurally never auto-acts, never a warning — consent is not hurried by a probe); meadow_consent_verify.py remains the read-only precondition checker",
        "guard": {"repo": "astrid", "file": "scripts/proactive_scan.py", "symbol": "_meadow_artifact_state"},
        "test": {"repo": "astrid", "kind": "python", "file": "scripts/proactive_scan.py",
                 "name": "ConstitutionHealthTests",
                 "run": "cd /Users/v/other/astrid && python3 scripts/proactive_scan.py --self-test"},
    },
    {
        "id": "hard_recovery_witness_probe",
        "shipped": "2026-09-03",
        "surface": "Constitution C4: minime health.json hard_recovery block {env_forced, fill, write_block_active} and the proactive_scan re-muffle detector",
        "failure_mode": "MINIME_HARD_RECOVERY_RESET used to blanket-block 24 homeostatic write fields, and any profile-read failure silently reverted to the blocking default with no witness — her dials refused with nothing saying so; guard = the engine witnesses the effective block state into health.json and probe_hard_recovery_witness WARNS when env_forced while the stable-core profile is active (the silent-fallback re-muffle) or when the block is active despite healthy fill; this row was owed since C4 shipped (the plan's guardrail matrix named it) and was missing until the 2026-09-03 completeness audit",
        "guard": {"repo": "astrid", "file": "scripts/proactive_scan.py", "symbol": "probe_hard_recovery_witness"},
        "test": {"repo": "astrid", "kind": "python", "file": "scripts/proactive_scan.py",
                 "name": "HardRecoveryWitnessTests",
                 "run": "cd /Users/v/other/astrid && python3 scripts/proactive_scan.py --self-test"},
    },
    {
