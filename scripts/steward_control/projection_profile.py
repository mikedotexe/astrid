"""Astrid-specific source-first DAG for the generic projection runtime."""

from __future__ import annotations

import sys

from .projection_types import ProjectionStep, command


def source_first_steps() -> tuple[ProjectionStep, ...]:
    python = sys.executable
    return (
        ProjectionStep(
            "addressing",
            (),
            (
                command(
                    python,
                    "scripts/introspection_addressing_audit.py",
                    "--workspace",
                    "{workspace}",
                    "--state-dir",
                    "{workspace}/diagnostics/introspection_addressing_v1",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("addressing",),
            ("introspections/introspection_*.txt",),
            (
                "diagnostics/introspection_addressing_v1/status.json",
                "diagnostics/introspection_addressing_v1/queue.md",
            ),
        ),
        ProjectionStep(
            "sandbox",
            ("addressing",),
            (
                command(
                    python,
                    "scripts/sandbox_trial_queue.py",
                    "--state-dir",
                    "{workspace}/diagnostics/sandbox_trial_queue_v1",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("addressing", "sandbox"),
            (
                "diagnostics/introspection_addressing_v1/status.json",
                "diagnostics/sandbox_trial_queue_v1/results/*.json",
            ),
            (
                "diagnostics/sandbox_trial_queue_v1/status.json",
                "diagnostics/sandbox_trial_queue_v1/queue.md",
            ),
        ),
        ProjectionStep(
            "corridor",
            ("addressing", "sandbox"),
            (
                command(
                    python,
                    "scripts/agency_corridor.py",
                    "--state-dir",
                    "{workspace}/diagnostics/agency_corridor_v1",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("addressing", "sandbox", "corridor_v1", "corridor_v2"),
            (
                "diagnostics/introspection_addressing_v1/status.json",
                "diagnostics/sandbox_trial_queue_v1/status.json",
            ),
            (
                "diagnostics/agency_corridor_v2/status.json",
                "diagnostics/agency_corridor_v2/leases.json",
                "diagnostics/agency_corridor_v2/queue.json",
                "diagnostics/agency_corridor_v2/programs.json",
                "diagnostics/agency_corridor_v2/report.md",
            ),
        ),
        ProjectionStep(
            "signal_spine",
            ("corridor",),
            (
                command(
                    python,
                    "scripts/signal_spine_projector.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("signal_spine",),
            (
                "diagnostics/signal_spine_v1/journeys/*.json",
                "diagnostics/signal_spine_v1/temporal_associations/*.json",
                "diagnostics/signal_spine_v1/capture_gaps/*.json",
            ),
            (
                "diagnostics/signal_spine_v1/projection_status.json",
                "diagnostics/signal_spine_v1/report.md",
            ),
        ),
        ProjectionStep(
            "contact_capacity_trace",
            ("signal_spine",),
            (
                command(
                    python,
                    "scripts/contact_capacity_trace.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("signal_spine", "contact_capacity_trace"),
            (
                "diagnostics/signal_spine_v1/journeys/*.json",
                "diagnostics/contact_capacity_trace_v1/contact_receipts/*.json",
                "diagnostics/contact_capacity_trace_v1/ingress_gaps/*.json",
            ),
            (
                "diagnostics/contact_capacity_trace_v2/status.json",
                "diagnostics/contact_capacity_trace_v2/traces.jsonl",
                "diagnostics/contact_capacity_trace_v2/ingress_gaps.jsonl",
                "diagnostics/contact_capacity_trace_v2/journey_origins.jsonl",
                "diagnostics/contact_capacity_trace_v2/capture_gaps.jsonl",
                "diagnostics/contact_capacity_trace_v2/report.md",
                "diagnostics/contact_capacity_trace_v1/status.json",
                "diagnostics/contact_capacity_trace_v1/traces.jsonl",
                "diagnostics/contact_capacity_trace_v1/orphan_journeys.jsonl",
                "diagnostics/contact_capacity_trace_v1/capture_gaps.jsonl",
                "diagnostics/contact_capacity_trace_v1/report.md",
            ),
        ),
        ProjectionStep(
            "lived_state_witness",
            ("signal_spine",),
            (
                command(
                    python,
                    "scripts/lived_state_witness.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("addressing", "signal_spine"),
            (
                "introspections/introspection_*.txt",
                "introspections/lived_state_witnesses/**/*.json",
                "environment_receipts/environment_receipts.jsonl",
                "../../../../neural-triple-reservoir/workspace/model_qos_receipts.jsonl",
            ),
            (
                "diagnostics/lived_state_witness_v1/status.json",
                "diagnostics/lived_state_witness_v1/witnesses.jsonl",
                "diagnostics/lived_state_witness_v1/auxiliary_artifacts.jsonl",
                "diagnostics/lived_state_witness_v1/artifact_integrity_issues.jsonl",
                "diagnostics/lived_state_witness_v1/gaps.jsonl",
                "diagnostics/lived_state_witness_v1/temporal_clusters.jsonl",
                "diagnostics/lived_state_witness_v1/concordance_clusters.jsonl",
                "diagnostics/lived_state_witness_v1/concordance_preflight.json",
                "diagnostics/lived_state_witness_v1/context_index.jsonl",
                "diagnostics/lived_state_witness_v1/report.md",
                "diagnostics/lived_state_witness_v1/migration_receipt.json",
            ),
        ),
        ProjectionStep(
            "grounded_introspection",
            ("addressing", "lived_state_witness"),
            (
                command(
                    python,
                    "scripts/grounded_introspection.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("addressing", "lived_state_witness", "grounded_introspection"),
            (
                "introspections/introspection_*.txt",
                "diagnostics/introspection_addressing_v1/status.json",
                "diagnostics/lived_state_witness_v1/witnesses.jsonl",
            ),
            (
                "diagnostics/grounded_introspection_v1/status.json",
                "diagnostics/grounded_introspection_v1/manifests.jsonl",
                "diagnostics/grounded_introspection_v1/claims.jsonl",
                "diagnostics/grounded_introspection_v1/discrepancies.jsonl",
                "diagnostics/grounded_introspection_v1/unbound_claims.jsonl",
                "diagnostics/grounded_introspection_v1/report.md",
            ),
        ),
        ProjectionStep(
            "reciprocal_uptake",
            ("lived_state_witness",),
            (
                command(
                    python,
                    "scripts/reciprocal_uptake.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("reciprocal_uptake",),
            ("../../../../shared/collaborations/correspondence_v1.jsonl",),
            (
                "diagnostics/reciprocal_uptake_v1/status.json",
                "diagnostics/reciprocal_uptake_v1/receipts.jsonl",
                "diagnostics/reciprocal_uptake_v1/current_receipts.jsonl",
                "diagnostics/reciprocal_uptake_v1/technical_context_migration_v1.json",
                "diagnostics/reciprocal_uptake_v1/current_view_migration_v2.json",
                "diagnostics/reciprocal_uptake_v1/context_identity_reconciliation_v2.json",
                "diagnostics/reciprocal_uptake_v1/report.md",
            ),
        ),
        ProjectionStep(
            "representation_contracts",
            ("lived_state_witness",),
            (
                command(
                    python,
                    "scripts/representation_contracts.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("representation_contracts",),
            (
                "../src/codec/projection.rs",
                "../src/codec/evidence_types.rs",
                "../src/astrid_shadow.rs",
                "../src/llm/provider/transport.rs",
                "../src/llm/provider/dialogue_runtime.rs",
                "../src/lived_state_witness/types.rs",
                "diagnostics/context_packing_pressure_v1.jsonl",
                "introspections/lived_state_witnesses/witnesses/*.json",
            ),
            (
                "diagnostics/representation_contracts_v1/status.json",
                "diagnostics/representation_contracts_v1/registry.jsonl",
                "diagnostics/representation_contracts_v1/transitions.jsonl",
                "diagnostics/representation_contracts_v1/report.md",
            ),
        ),
        ProjectionStep(
            "domain_boundary_audit",
            ("representation_contracts",),
            (
                command(
                    python,
                    "scripts/domain_boundary_audit.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("representation_contracts", "domain_boundary_audit"),
            (
                "../domain_boundaries_v1.toml",
                "../domain_boundaries_legacy_large_files_v1.json",
                "../DOMAIN_BOUNDARIES.md",
                "../src/**/*.rs",
            ),
            (
                "diagnostics/domain_boundary_audit_v1/status.json",
                "diagnostics/domain_boundary_audit_v1/large_files.jsonl",
                "diagnostics/domain_boundary_audit_v1/violations.jsonl",
                "diagnostics/domain_boundary_audit_v1/report.md",
            ),
        ),
        ProjectionStep(
            "claim_families",
            ("addressing", "signal_spine"),
            (
                command(
                    python,
                    "scripts/claim_families.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("addressing", "claim_families"),
            (
                "diagnostics/introspection_addressing_v1/status.json",
                "environment_receipts/latest_environment_receipt.json",
            ),
            (
                "diagnostics/claim_families_v1/status.json",
                "diagnostics/claim_families_v1/report.md",
                "diagnostics/claim_families_v1/migration_receipt.json",
            ),
        ),
        ProjectionStep(
            "experiment_dossiers",
            ("sandbox", "claim_families"),
            (
                command(
                    python,
                    "scripts/experiment_dossiers.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("sandbox", "claim_families"),
            (
                "diagnostics/sandbox_trial_queue_v1/status.json",
                "diagnostics/claim_families_v1/status.json",
                "diagnostics/lived_state_witness_v1/context_index.jsonl",
            ),
            (
                "diagnostics/experiment_dossiers_v1/status.json",
                "diagnostics/experiment_dossiers_v1/report.md",
            ),
        ),
        ProjectionStep(
            "authority_temporal",
            ("experiment_dossiers",),
            (
                command(
                    python,
                    "scripts/authority_temporal_audit.py",
                    "--workspace",
                    "{workspace}",
                    "--json",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("authority_lifecycle",),
            ("action_threads/threads/*/authority_gate.jsonl",),
            ("diagnostics/authority_temporal_v1/status.json",),
        ),
        ProjectionStep(
            "model_qos",
            ("authority_temporal",),
            (
                command(
                    python,
                    "scripts/model_qos_projector.py",
                    "--workspace",
                    "{workspace}",
                    "--json",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("model_qos",),
            (
                "../../../../neural-triple-reservoir/workspace/model_qos_receipts.jsonl",
            ),
            ("diagnostics/model_qos_v1/status.json",),
        ),
        ProjectionStep(
            "evidence_study_runtime",
            (
                "experiment_dossiers",
                "model_qos",
                "reciprocal_uptake",
                "representation_contracts",
            ),
            (
                command(
                    python,
                    "scripts/evidence_study.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            (
                "felt_mechanism_concordance",
                "signal_spine",
                "lived_state_witness",
                "model_qos",
                "reciprocal_uptake",
                "representation_contracts",
            ),
            (
                "diagnostics/evidence_study_runtime_v1/operator_events.jsonl",
                "diagnostics/evidence_study_runtime_v1/samples/*.jsonl",
                "../../../scripts/evidence_study_runtime/manifests/astrid_baseline_campaigns_v1.json",
            ),
            (
                "diagnostics/evidence_study_runtime_v1/status.json",
                "diagnostics/evidence_study_runtime_v1/campaigns.jsonl",
                "diagnostics/evidence_study_runtime_v1/plans.jsonl",
                "diagnostics/evidence_study_runtime_v1/comparisons.jsonl",
                "diagnostics/evidence_study_runtime_v1/report.md",
            ),
        ),
        ProjectionStep(
            "counterfactual_change_lab",
            ("evidence_study_runtime", "representation_contracts"),
            (
                command(
                    python,
                    "scripts/counterfactual_change_lab.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("evidence_study_runtime", "representation_contracts", "counterfactual_change_lab"),
            (
                "../../../scripts/counterfactual_change_lab/fixtures/codec_basis_public_corpus_v1.json",
                "../../../scripts/counterfactual_change_lab/fixtures/codec_basis_public_embeddings_v1.json",
                "../src/codec/projection.rs",
            ),
            (
                "diagnostics/counterfactual_change_lab_v1/status.json",
                "diagnostics/counterfactual_change_lab_v1/campaigns/fixed_projection_basis_epoch_comparison_v1/campaign_manifest.json",
                "diagnostics/counterfactual_change_lab_v1/campaigns/fixed_projection_basis_epoch_comparison_v1/comparison.json",
                "diagnostics/counterfactual_change_lab_v1/campaigns/fixed_projection_basis_epoch_comparison_v1/basis_metrics.jsonl",
                "diagnostics/counterfactual_change_lab_v1/campaigns/fixed_projection_basis_epoch_comparison_v1/evidence_study_handoff.json",
                "diagnostics/counterfactual_change_lab_v1/report.md",
            ),
        ),
        ProjectionStep(
            "felt_mechanism_concordance",
            (
                "experiment_dossiers",
                "model_qos",
                "reciprocal_uptake",
                "representation_contracts",
                "evidence_study_runtime",
            ),
            (
                command(
                    python,
                    "scripts/felt_mechanism_concordance.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            (
                "signal_spine",
                "lived_state_witness",
                "claim_families",
                "sandbox",
                "model_qos",
                "reciprocal_uptake",
                "representation_contracts",
                "felt_mechanism_concordance",
            ),
            (
                "diagnostics/felt_mechanism_concordance_v1/operator_events.jsonl",
                "diagnostics/experiment_dossiers_v1/status.json",
                "diagnostics/lived_state_witness_v1/context_index.jsonl",
                "diagnostics/model_qos_v1/status.json",
                "diagnostics/reciprocal_uptake_v1/status.json",
                "diagnostics/representation_contracts_v1/status.json",
            ),
            (
                "diagnostics/felt_mechanism_concordance_v1/status.json",
                "diagnostics/felt_mechanism_concordance_v1/studies.jsonl",
                "diagnostics/felt_mechanism_concordance_v1/observations.jsonl",
                "diagnostics/felt_mechanism_concordance_v1/results.jsonl",
                "diagnostics/felt_mechanism_concordance_v1/report.md",
            ),
        ),
        ProjectionStep(
            "agency_commons",
            ("felt_mechanism_concordance",),
            (
                command(
                    python,
                    "scripts/agency_commons.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("agency_commons",),
            (
                "../../../../shared/collaborations/phase_transitions_v1.jsonl",
                "../../../../shared/collaborations/correspondence_v1.jsonl",
                "sovereignty_proposals.json",
                "agency_requests/*.json",
                "diagnostics/agency_commons_v1/operator_events.jsonl",
            ),
            (
                "diagnostics/agency_commons_v1/status.json",
                "diagnostics/agency_commons_v1/records.jsonl",
                "diagnostics/agency_commons_v1/report.md",
            ),
        ),
        ProjectionStep(
            "division_chronicle",
            (),
            (
                command(
                    python,
                    "scripts/division_ceremony_chronicle.py",
                    "--workspace",
                    "{workspace}/../../../../minime/workspace",
                    "--output",
                    "{workspace}/../../../../minime/workspace/division/chronicle",
                    "project",
                ),
            ),
            (),
            (
                "../../../../minime/workspace/division/ceremony_v1.jsonl",
                "../../../../minime/workspace/division/events.jsonl",
                "../../../../minime/workspace/division/status.json",
                "../../../../minime/workspace/division/runtime-manifest.json",
                "../../../../minime/workspace/division/runtime/*.json",
                "../../../../minime/workspace/division/runtime/events.jsonl",
                "../../../../minime/workspace/division/runtime/receipts/*.json",
                "../../../../minime/workspace/division/followup/cycle_v1.json",
                "../../../../minime/workspace/division/followup/events_v1.jsonl",
                "../../../../minime/minime/src/runtime.rs",
                "../../../../minime/workspace/reservoir/*/status.json",
                "reservoir/*/status.json",
            ),
            (
                "../../../../minime/workspace/division/chronicle/chronicle_v1.json",
                "../../../../minime/workspace/division/chronicle/chronicle_v1.html",
            ),
        ),
        ProjectionStep(
            "passage_observatory",
            ("agency_commons", "division_chronicle"),
            (
                command(
                    python,
                    "scripts/passage_observatory.py",
                    "--division-workspace",
                    "{workspace}/../../../../minime/workspace",
                    "--phase-ledger",
                    "{workspace}/../../../../shared/collaborations/phase_transitions_v1.jsonl",
                    "--output",
                    "{workspace}/../../../../minime/workspace/division/passage-observatory",
                    "project",
                    "--receipt-json",
                ),
            ),
            ("agency_commons",),
            (
                "../../../../shared/collaborations/phase_transitions_v1.jsonl",
            ),
            (
                "../../../../minime/workspace/division/passage-observatory/observatory_v1.json",
                "../../../../minime/workspace/division/passage-observatory/observatory_v1.html",
                "../../../../minime/workspace/division/passage-observatory/observatory_v2.json",
                "../../../../minime/workspace/division/passage-observatory/observatory_v2.html",
            ),
        ),
        ProjectionStep(
            "temporal_bearing",
            (
                "contact_capacity_trace",
                "lived_state_witness",
                "passage_observatory",
            ),
            (
                command(
                    python,
                    "scripts/temporal_bearing.py",
                    "--workspace",
                    "{workspace}",
                    "--minime-workspace",
                    "{workspace}/../../../../minime/workspace",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            (
                "signal_spine",
                "contact_capacity_trace",
                "lived_state_witness",
                "agency_commons",
            ),
            (
                "diagnostics/signal_spine_v1/journeys/*.json",
                "diagnostics/contact_capacity_trace_v2/traces.jsonl",
                "diagnostics/contact_capacity_trace_v2/ingress_gaps.jsonl",
                "diagnostics/contact_capacity_trace_v1/contact_receipts/*.json",
                "diagnostics/lived_state_witness_v1/witnesses.jsonl",
                "shadow_cartography/trajectory_*.json",
                "volition_v1/astrid/inquiries/receipts/*.json",
                "../../../../minime/workspace/division/passage-observatory/observatory_v1.json",
                "../../../../minime/workspace/owner_inquiry/receipts/*.json",
                "../../../../minime/workspace/inquiries/receipts/*.json",
            ),
            (
                "diagnostics/temporal_bearing_v1/status.json",
                "diagnostics/temporal_bearing_v1/latest.json",
                "diagnostics/temporal_bearing_v1/indexes.json",
                "diagnostics/temporal_bearing_v1/gaps.jsonl",
                "diagnostics/temporal_bearing_v1/unlinked_evidence.jsonl",
                "diagnostics/temporal_bearing_v1/report.md",
            ),
        ),
        ProjectionStep(
            "felt_contracts",
            (
                "experiment_dossiers",
                "corridor",
                "signal_spine",
                "lived_state_witness",
                "reciprocal_uptake",
                "representation_contracts",
                "authority_temporal",
                "model_qos",
                "felt_mechanism_concordance",
                "agency_commons",
            ),
            (
                command(
                    python,
                    "scripts/felt_contract_graph.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--actor",
                    "steward-control-projector",
                    "--receipt-json",
                    "--json",
                ),
            ),
            (
                "addressing",
                "sandbox",
                "corridor_v1",
                "corridor_v2",
                "signal_spine",
                "lived_state_witness",
                "reciprocal_uptake",
                "representation_contracts",
                "claim_families",
                "felt_mechanism_concordance",
                "agency_commons",
                "authority_lifecycle",
                "model_qos",
                "felt_contracts",
            ),
            (
                "diagnostics/claim_families_v1/status.json",
                "diagnostics/experiment_dossiers_v1/status.json",
                "environment_receipts/environment_receipts.jsonl",
            ),
            (
                "diagnostics/felt_contract_graph_v1/status.json",
                "diagnostics/felt_contract_graph_v1/contracts.jsonl",
                "diagnostics/felt_contract_graph_v1/report.md",
                "diagnostics/felt_contract_graph_v1/migration_receipt.json",
            ),
        ),
        ProjectionStep(
            "steward_work_selection",
            ("felt_contracts",),
            (
                command(
                    python,
                    "scripts/steward_work_selection.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("felt_contracts", "steward_work_selection"),
            (
                "diagnostics/felt_contract_graph_v1/contracts.jsonl",
                "diagnostics/attention_portfolio_v1/pin_events.jsonl",
                "diagnostics/attention_portfolio_v2/pin_events.jsonl",
                "diagnostics/steward_work_selection_v1/priority_events.jsonl",
            ),
            (
                "diagnostics/steward_work_selection_v1/status.json",
                "diagnostics/steward_work_selection_v1/selection.json",
                "diagnostics/steward_work_selection_v1/report.md",
            ),
        ),
        ProjectionStep(
            "living_problem_registry",
            (
                "felt_contracts",
                "steward_work_selection",
            ),
            (
                command(
                    python,
                    "scripts/living_problem_registry.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            (
                "addressing",
                "sandbox",
                "felt_contracts",
                "steward_work_selection",
            ),
            (
                "diagnostics/felt_contract_graph_v1/contracts.jsonl",
                "diagnostics/introspection_addressing_v1/status.json",
                "diagnostics/sandbox_trial_queue_v1/status.json",
                "diagnostics/steward_work_selection_v1/selection.json",
                "diagnostics/experiment_dossiers_v1/status.json",
                "diagnostics/authority_temporal_v1/status.json",
            ),
            (
                "diagnostics/living_problem_registry_v1/status.json",
                "diagnostics/living_problem_registry_v1/problems.jsonl",
                "diagnostics/living_problem_registry_v1/changed_problems.jsonl",
                "diagnostics/living_problem_registry_v1/queue.md",
                "diagnostics/living_problem_registry_v1/report.md",
                "diagnostics/living_problem_registry_v2/status.json",
                "diagnostics/living_problem_registry_v2/problems.jsonl",
                "diagnostics/living_problem_registry_v2/changed_problems.jsonl",
                "diagnostics/living_problem_registry_v2/queue.md",
                "diagnostics/living_problem_registry_v2/report.md",
            ),
        ),
        ProjectionStep(
            "felt_constellation",
            ("claim_families", "felt_contracts", "living_problem_registry"),
            (
                command(
                    python,
                    "scripts/felt_constellation.py",
                    "--workspace",
                    "{workspace}",
                    "project",
                    "--write",
                    "--receipt-json",
                ),
            ),
            ("claim_families", "felt_contracts", "living_problem_registry", "felt_constellation"),
            (
                "diagnostics/claim_families_v1/status.json",
                "diagnostics/felt_contract_graph_v1/contracts.jsonl",
                "diagnostics/living_problem_registry_v2/problems.jsonl",
                "../../../scripts/felt_constellation/policy_v1.json",
            ),
            (
                "diagnostics/felt_constellation_v1/status.json",
                "diagnostics/felt_constellation_v1/constellations.jsonl",
                "diagnostics/felt_constellation_v1/memberships.jsonl",
                "diagnostics/felt_constellation_v1/unmatched_claims.jsonl",
                "diagnostics/felt_constellation_v1/report.md",
            ),
        ),
        ProjectionStep(
            "experiential_epistemics",
            (
                "contact_capacity_trace",
                "counterfactual_change_lab",
                "domain_boundary_audit",
                "felt_contracts",
                "felt_constellation",
                "evidence_study_runtime",
                "grounded_introspection",
                "living_problem_registry",
                "passage_observatory",
                "steward_work_selection",
                "temporal_bearing",
            ),
            (
                command(
                    python,
                    "scripts/experiential_epistemics.py",
                    "--workspace",
                    "{workspace}",
                    "lint",
                    "--write",
                    "--receipt-json",
                ),
            ),
            (
                "felt_mechanism_concordance",
                "felt_contracts",
                "steward_work_selection",
            ),
            (
                "diagnostics/evidence_study_runtime_v1/*.json",
                "diagnostics/evidence_study_runtime_v1/*.jsonl",
                "diagnostics/felt_mechanism_concordance_v1/*.jsonl",
                "diagnostics/felt_contract_graph_v1/contracts.jsonl",
                "diagnostics/living_problem_registry_v1/*.json",
                "diagnostics/living_problem_registry_v1/*.jsonl",
                "diagnostics/living_problem_registry_v2/*.json",
                "diagnostics/living_problem_registry_v2/*.jsonl",
                "diagnostics/steward_work_selection_v1/*.json",
            ),
            ("diagnostics/experiential_epistemics_v1/status.json",),
        ),
    )
