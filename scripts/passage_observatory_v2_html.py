...
function renderMoments() {
  ...
  `<article class="moment"><div><span class="dot ${esc(item.rail)}"></span><strong>${title(item.rail)}</strong><br><time class="meta">${esc(stamp(item.recorded_at_unix_ms))}</time></div><div><strong>${title(item.event_kind)}</strong><br><span class="meta">${esc(item.actor || "observed")} · ${esc(title(item.source_provenance?.source_category || "opaque boundary"))}</span></div><div><code>${esc(item.source_record_ref)}</code><br><span class="meta">${esc(Object.entries(item.state).map(([key, value]) => `${title(key)}: ${title(value)}`).join(" · ") || "No categorical state")}</span><br><span class="meta">${esc(title(item.source_provenance?.role || "unclassified observatory event"))} · ${esc(title(item.source_provenance?.boundary_status || "opaque boundary needs description"))}</span></div><code class="token">${esc(item.reference_token)}</code></article>`
