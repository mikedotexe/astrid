...
grep -q '^PrivateTmp=no$' "$state_migration_recover_unit" || fail "pre-mount migration recovery retains the tmpfiles/local-fs ordering cycle"
! grep -q '^PrivateTmp=yes$' "$state_migration_recover_unit" || fail "pre-mount migration recovery is ordered after tmpfiles"
...
grep -q "ConditionFileNotEmpty=%s" "$REPO_ROOT/packaging/systemd/root/migrate-edge-user-services-to-system" || fail "root-manager handoff marker is not gated by a systemd-249-compatible nonempty-file condition"
! grep -q 'ConditionPathIsRegular' "$REPO_ROOT/packaging/systemd/root/migrate-edge-user-services-to-system" || fail "root-manager handoff marker uses a condition unsupported by systemd 249"
