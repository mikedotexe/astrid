...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("command", choices=("project", "verify", "report", "show", "pin", "unpin"))
    parser.add_argument("--write", action="store_true")
    parser.add_argument("--receipt-json", action="store_true")
    parser.add_argument("--being", choices=("astrid", "minime"))
    ...
    try:
        if args.command in {"pin", "unpin"}:
            pin = BeingImportancePinV1.build(being=args.being, contract_id=args.contract_id,
                                             action=args.command, source_event_id=args.source_event_id,
        ...
        else: value = query(workspace, args.contract_id)
